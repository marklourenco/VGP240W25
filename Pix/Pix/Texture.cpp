...
#pragma pack(pop)
    ...
    X::Color GetBilinearFilterPixelColor(const Texture& tex, float u, float v)
    {
        ...
        // step 4: get the opposite ratios (eg. if ratio is 0.4, uOpposite is 0.6)
...
void Texture::Load(const std::string& fileName)
{
    ...
    {
        char buffer[128];
        sprintf_s(buffer, "Failed to open texture file: %s", fileName.c_str());
    ...
    {
        fclose(file);
        MessageBoxA(nullptr, "Only 24-bit aligned bitmaps are supported", "Texture Error", MB_OK || MB_ICONEXCLAMATION);
        return;
    }
